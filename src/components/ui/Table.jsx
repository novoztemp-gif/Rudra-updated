    ...
    <table className="w-full text-sm">
      <thead>
        <tr className="bg-brand-50/60 border-b border-gray-200">
          {columns.map((c) => <th key={c.key} className={`px-4 py-3 text-left text-xs font-semibold text-brand-800 uppercase tracking-wider ${c.align === "right" ? "text-right" : ""}`}>{c.label}</th>)}
        </tr>
      </thead>
      <tbody className="divide-y divide-gray-50">
        {data.length === 0 ? (
          ...
          <tr key={row.id || i} className={`${onRowClick ? "cursor-pointer hover:bg-brand-50/40" : ""} transition-colors`} onClick={() => onRowClick?.(row)}>
